...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
        ...
        <form action="{{ route('admin.monitoring-sholat.riwayat') }}"
              method="GET"
            ...
            <div>
                ...
                <select name="bulan"
                    ...
                    @foreach($daftarBulan as $nomorBulan => $namaBulan)
                        <option value="{{ $nomorBulan }}" {{ $bulanAngka == $nomorBulan ? 'selected' : '' }}>
                            {{ $namaBulan }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div class="xl:col-span-1 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
            ...
            <div class="grid grid-cols-7 gap-2">
                ...
                @foreach($kalender as $hari)
                    ...
                    @php
                        ...
                        if ($hari['total'] > 0) {
                            $dayClass = 'bg-[#EEF7F1] border-[#DDF3E7] hover:bg-[#DDF3E7]';
                            $textClass = 'text-[#2F7D55]';
                            $smallText = $hari['total'] . ' data';
                        }

                        if ($hari['is_selected']) {
                            $dayClass = 'bg-[#2F7D55] border-[#2F7D55] text-white shadow-md';
                            $textClass = 'text-white';
                            $smallText = $hari['total'] > 0 ? $hari['total'] . ' data' : 'kosong';
                        }
                    @endphp

                    <a href="{{ route('admin.monitoring-sholat.riwayat', array_filter([
                            'kelas_id' => $kelasId,
                            'bulan' => $bulanAngka,
                            'tahun' => $tahun,
                       ...
                       class="relative min-h-[84px] rounded-2xl border p-3 transition {{ $dayClass }}">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if($hari['is_today'])
                                <span class="w-2 h-2 rounded-full {{ $hari['is_selected'] ? 'bg-white' : 'bg-[#2F7D55]' }}"></span>
                            @endif
                        </div>

                        <p class="text-[11px] mt-5 {{ $hari['is_selected'] ? 'text-white/80' : 'text-gray-400' }}">
                            {{ $smallText }}
                        </p>
            ...
            <div class="flex flex-wrap gap-4 mt-6 text-sm">
                <div class="flex items-center gap-2">
                    <span class="w-4 h-4 rounded-lg bg-[#EEF7F1] border border-[#DDF3E7]"></span>
                    <span class="text-gray-500">Ada data</span>
                </div>
                ...
                <div class="flex items-center gap-2">
                    <span class="w-4 h-4 rounded-lg bg-[#2F7D55]"></span>
        ...
        <div class="xl:col-span-2 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-7">
                ...
                <a href="{{ route('admin.monitoring-sholat.index', array_filter([
                    'kelas_id' => $kelasId,
